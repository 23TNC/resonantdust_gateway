...
//!
//! Recipe *semantics* (input match + output plan) moved to the shared recipe
...
fn card_view(c: &crate::bindings::shard::Card) -> CardView {
    CardView {
        card_id: c.card_id,
        owner_id: c.owner_id,
...
impl CardStore for Snapshot {
    fn card_at(&self, card_id: u32, _time_ms: u64) -> Option<CardView> {
        self.cards.get(&card_id).map(card_view)
    }
}

impl StackStore for Snapshot {
        ...
        self.cards
            ...
            .filter(|c| {
                micro_is_card(c.flags)
                    && matches!(Micro::of(c.micro_location, c.flags), Micro::Stacked { root, .. } if root == root_id)
            })
            .map(card_view)
            .collect()
    }
